...
- `api/like.js`, `api/view.js` — 좋아요·조회수 저장 함수
...
| `Insight` | 텍스트 | 권장 | 내 생각·정리. **비워두면 그 페이지 본문 전체를 자동으로 읽어옴** |
...
| `Views` | 숫자(Number) | 선택 | 조회수 저장용. 만들면 "읽음 N" 표시됨 |
| `Likes` | 숫자(Number) | 선택 | 좋아요 수 저장용 |

> **Status 값 만들기:** Status 열 클릭 → 옵션에 `홈페이지 게시` 추가. 이 값이 정확히 있어야 사이트에 뜹니다. (`미완` 같은 다른 값은 자동으로 숨겨짐)

> **인사이트 쓰는 법 2가지 —** ① `Insight` 칸에 직접 텍스트로 적기, 또는 ② 칸을 비우고 그 글의 **노션 페이지 본문**에 정리하기. 지금 노션에 쓰던 방식 그대로 쓰셔도 됩니다.
...
새 로고가 준비되면 아래 파일만 같은 이름으로 덮어쓰면 됩니다. 코드 수정 불필요.

| 파일 | 용도 | 권장 크기 |
|---|---|---|
| `assets/logo-mark.png` | 헤더 좌측 심볼 | 정사각형 512×512 |
| `assets/logo-full.png` | 워드마크 (가로형) | 가로 800px 내외 |
