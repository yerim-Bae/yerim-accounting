...
현재 아이콘은 잉크색 사각형 + 아이보리 `Y`(아래 줄기만 오렌지)로, 코드로 그려 만들었습니다.
새 로고가 준비되면 아래 파일을 같은 이름으로 덮어쓰면 됩니다. 코드 수정은 필요 없습니다.

| 파일 | 용도 | 권장 크기 |
|---|---|---|
| `assets/favicon.svg` | 브라우저 탭 아이콘 (최신 브라우저가 우선 사용) | 정사각형 벡터 |
| `assets/favicon-32.png` | 탭 아이콘 (구형 대비) | 32×32 |
...
아이콘 파일을 바꿨는데 예전 그림이 계속 보이면, 두 HTML 파일 안의 `?v=6` 을 `?v=7` 로
올려주세요. 브라우저가 아이콘을 강하게 캐시해서 그렇습니다.
